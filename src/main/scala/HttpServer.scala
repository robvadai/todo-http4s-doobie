...
import service.{SourceCodeService, TodoService}
...
object HttpServer {
  ...
  private def create(resources: Resources)(implicit concurrentEffect: ConcurrentEffect[IO], timer: Timer[IO]): IO[ExitCode] = {
    ...
    val repository = new SourceCodeRepository(resources.transactor)

    for {
//      _ <- Database.initialize(resources.transactor)
//      repository = new SourceCodeRepository(resources.transactor)
      exitCode <- BlazeServerBuilder[IO](global)
        .bindHttp(resources.config.server.port, resources.config.server.host)
        .withHttpApp(new SourceCodeService(repository).routes.orNotFound).serve.compile.lastOrError
    } yield exitCode
  }
